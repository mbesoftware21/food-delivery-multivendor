...
-- Create composite type for the result
CREATE TYPE public.restaurant_by_owner_result AS (
    _id text,
    email text,
    "userType" text,
    restaurants jsonb
...
-- Recreate function with composite type return
CREATE OR REPLACE FUNCTION public.restaurant_by_owner(owner_id_input text)
RETURNS SETOF public.restaurant_by_owner_result
LANGUAGE plpgsql
STABLE
AS $$
BEGIN
